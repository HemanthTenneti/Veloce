#!/usr/bin/env python3
"""NEXUS Reignite V4 Final — Production-grade DocType creation for Veloce.

V4 Gold Standard changes vs V3:
  - thumbnail  : Attach Image / Required  (was: Data)
  - image_1..10: Attach Image / Optional  (was: Data)
  - FIELD_RENAMES removed — this script is a clean drop-in, not a migration patch.

Placement inside the Frappe container:
    /home/frappe/frappe-bench/apps/frappe/frappe/reignite_doctypes_v4_final.py

Execution:
    bench --site localhost execute frappe.reignite_doctypes_v4_final.run_from_bench
"""
...
def run_from_bench() -> None:
    """Entrypoint for: bench --site localhost execute frappe.reignite_doctypes_v4_final.run_from_bench"""
    print("=" * 70)
    print("NEXUS Reignite V4 Final — Veloce Production DocType Engine")
    print("=" * 70)
    print()

    for spec in DOC_TYPES:
        result = _create_doctype(spec)
        print(result)
    ...
    print()
    print("=" * 70)
    print("V4 Final complete — all DocTypes synced, DB committed.")
    print("Next: bench --site localhost migrate && bench --site localhost clear-cache")
    print("=" * 70)
